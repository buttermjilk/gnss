...
eth_hdop_history = deque(maxlen=TREND_WINDOW)
...
def check_precision(state):
    ...
    if not usb or not eth:
        return alerts

    usb_hdop = usb.get("hdop")
    eth_hdop = eth.get("hdop")

    if usb_hdop is None or eth_hdop is None:
        return alerts

    now = time.time()

    usb_hdop_history.append(usb_hdop)
    eth_hdop_history.append(eth_hdop)
...
# USB HDOP signal ratio
    ...
    ratio = usb_hdop / max(eth_hdop, 0.1)
    ...
    if bad_ratio_count >= 3:
        alerts.append(
            f"USB HDOP significantly worse than ETH (ratio {ratio:.2f})"
        )
        bad_ratio_count = 0
